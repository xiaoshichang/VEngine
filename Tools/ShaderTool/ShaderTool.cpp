...
void PrintHelp()
{
    std::cout
        ...
        << "\n"
        << "Commands:\n"
        << "  compile --source <file> --output <dir> --name <shader> [--dxc <path>] [--fxc <path>] [--spirv-cross <path>]\n"
        << "  --help\n";
}
...
std::optional<std::string> ValidateExplicitBindings(std::string_view source)
{
    ...
    while(std::getline(input, line))
    {
        ...
        const bool hasBindableDeclaration =
            std::regex_search(strippedLine, cbufferDeclaration) ||
...
std::filesystem::path BuildArtifactPath(
