...
set(VE_DXC_EXECUTABLE "" CACHE FILEPATH "Path to the dxc executable used for D3D12 DXIL output.")
...
function(ve_reset_old_build_local_dxc_cache)
    ...
    if(NOT EXISTS "${VE_DXC_EXECUTABLE}")
        set(VE_DXC_EXECUTABLE "" CACHE FILEPATH "Path to the dxc executable used for D3D12 DXIL output." FORCE)
        return()
    endif()
...
function(ve_setup_directx_shader_compiler)
    ...
    if(NOT VE_DXC_EXECUTABLE)
        set(projectDxcExecutable "${VE_DXC_THIRD_PARTY_ROOT}/dxc.exe")

        if(EXISTS "${projectDxcExecutable}")
            set(VE_DXC_EXECUTABLE "${projectDxcExecutable}" CACHE FILEPATH "Path to the dxc executable used for D3D12 DXIL output." FORCE)
        endif()
    endif()

    if(NOT VE_DXC_EXECUTABLE)
        find_program(foundDxcExecutable
            NAMES dxc dxc.exe
            DOC "Path to the dxc executable used for D3D12 DXIL output."
        )

        if(foundDxcExecutable)
            set(VE_DXC_EXECUTABLE "${foundDxcExecutable}" CACHE FILEPATH "Path to the dxc executable used for D3D12 DXIL output." FORCE)
        endif()
    endif()
    ...
    if(NOT VE_DXC_EXECUTABLE)
        message(FATAL_ERROR
            "dxc was not found. Set VE_DXC_EXECUTABLE to a D3D12 DXIL-capable dxc executable "
            "or install the project DXC payload under ThirdParty/DirectXShaderCompiler."
        )
    endif()
