...
namespace
{
    ...
    bool TestRenderTarget()
    {
        ...
        passed &= Expect(target.GetExtent().width == 1280 && target.GetExtent().height == 720,
                         "Render target extent should persist");
    ...
    bool TestViewportClientSync()
    {
        ...
        passed &= Expect(viewportClient.GetRenderTarget().GetKind() == ve::RenderTargetKind::Window,
                         "ViewportClient should bind a window render target");
        passed &= Expect(viewportClient.GetRenderTarget().GetExtent().width == 1920 &&
                             viewportClient.GetRenderTarget().GetExtent().height == 1080,
                         "ViewportClient should sync extent from window");
        passed &= Expect(viewportClient.GetNativeWindow() == window.nativeWindow,
                         "ViewportClient should preserve native window handle");
        passed &= Expect(viewportClient.GetNativeLayer() == window.nativeLayer,
                         "ViewportClient should preserve native layer handle");
        ...
        passed &= Expect(viewportClient.GetRenderTarget().GetKind() == ve::RenderTargetKind::Texture,
                         "Off-screen viewport should normalize the render target kind");
        passed &= Expect(viewportClient.GetBindingKind() == ve::ViewportBindingKind::RenderTarget,
                         "ViewportClient should report off-screen binding kind");

        viewportClient.ResetBinding();
        passed &= Expect(!viewportClient.IsBound(), "ViewportClient reset should clear the binding");
        passed &= Expect(viewportClient.GetBindingKind() == ve::ViewportBindingKind::None,
                         "ViewportClient reset should return to an unbound state");
