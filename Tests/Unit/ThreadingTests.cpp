...
namespace
{
    ...
    bool TestThreadStartJoinAndName()
    {
        ...
        passed &=
            Expect(thread.GetName() == std::string_view("ThreadingTestWorker"), "Thread should preserve its name");
    ...
    bool TestThreadRejectsRepeatedStart()
    {
        ...
        passed &= ExpectOk(thread.Start(ve::ThreadDesc{"RepeatedStartWorker"}, [&]() { releaseWorker.Wait(); }),
                           "Initial thread start should succeed");

        const ve::ErrorCode repeatedStart = thread.Start(ve::ThreadDesc{"ShouldFail"}, []() {});

        passed &=
            Expect(repeatedStart != ve::ErrorCode::None, "Repeated Start should fail while the thread is joinable");
    ...
    bool TestMutexAndLockGuard()
    {
        ...
        {
            passed &= ExpectOk(threads[threadIndex].Start(ve::ThreadDesc{"CounterWorker"},
                                                          [&]()
                                                          {
                                                              for (int iteration = 0; iteration < IterationsPerThread;
                                                                   ++iteration)
                                                              {
                                                                  ve::LockGuard lock(mutex);
                                                                  ++counter;
        ...
        passed &= Expect(counter == ThreadCount * IterationsPerThread,
    ...
    bool TestConditionVariable()
    {
        ...
        ve::Thread waiter;
        passed &= ExpectOk(waiter.Start(ve::ThreadDesc{"ConditionWaiter"},
                                        [&]()
                                        {
    ...
    bool TestSemaphore()
    {
        ...
        ve::Thread worker;
        passed &= ExpectOk(worker.Start(ve::ThreadDesc{"SemaphoreWorker"},
                                        [&]()
                                        {
                                            semaphore.Acquire();
    ...
    bool TestManualResetEvent()
    {
        ...
        passed &= ExpectOk(first.Start(ve::ThreadDesc{"ManualEventFirst"},
                                       [&]()
                                       {
                                           event.Wait();
        ...
        passed &= ExpectOk(second.Start(ve::ThreadDesc{"ManualEventSecond"},
                                        [&]()
                                        {
                                            event.Wait();
                                            wakeCount.fetch_add(1, std::memory_order_acq_rel);
    ...
    bool TestAutoResetEvent()
    {
        ...
        passed &= ExpectOk(worker.Start(ve::ThreadDesc{"AutoEventWorker"},
                                        [&]()
                                        {
                                            event.Wait();
                                            wakeCount.fetch_add(1, std::memory_order_acq_rel);
    ...
    bool TestLockFreeSpscQueueCrossThread()
    {
        ...
        passed &= ExpectOk(consumer.Start(ve::ThreadDesc{"SpscConsumer"},
                                          [&]()
                                          {
                                              int expected = 1;
        ...
        passed &= ExpectOk(producer.Start(ve::ThreadDesc{"SpscProducer"},
                                          [&]()
                                          {
    ...
    bool TestLockFreeMpscQueueMultipleProducers()
    {
        ...
        for (int producerIndex = 0; producerIndex < ProducerCount; ++producerIndex)
        {
            ...
            passed &= ExpectOk(producers[producerIndex].Start(
                                   ve::ThreadDesc{"MpscProducer"},
                                   [&, baseValue]()
                                   {
        ...
        passed &=
            Expect(producerFailures.load(std::memory_order_acquire) == 0, "MPSC producers should not fail pushes");
