...
bool TestResetDefaults()
{
    ...
    passed &= Expect(snapshot.maxDeltaSeconds == ve::Time::DefaultMaxDeltaSeconds, "Reset should restore max delta");
    passed &= Expect(snapshot.fixedDeltaSeconds == ve::Time::DefaultFixedDeltaSeconds, "Reset should restore fixed delta");
...
bool TestAdvanceUpdatesFrameAndClampsDelta()
{
    ...
    passed &= Expect(
        NearlyEqual(snapshot.totalSeconds, 0.016 + ve::Time::DefaultMaxDeltaSeconds),
...
bool TestFixedStepAccumulator()
{
    ...
    passed &= Expect(
        NearlyEqual(snapshot.fixedAccumulatorSeconds, 0.05, 1.0e-5),
    ...
    passed &= Expect(
        NearlyEqual(snapshot.fixedAccumulatorSeconds, 0.09, 1.0e-5),
    ...
    passed &= Expect(
        NearlyEqual(snapshot.fixedAccumulatorSeconds, 0.01, 1.0e-5),
