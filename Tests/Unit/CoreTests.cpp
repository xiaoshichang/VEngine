...
bool TestCoreTypesAndMacros()
{
    ...
    passed &= Expect((VE_BUILD_DEBUG + VE_BUILD_RELEASE) == 1, "Exactly one build configuration macro should be set");
    passed &= Expect((VE_COMPILER_MSVC || VE_COMPILER_CLANG || VE_COMPILER_GCC), "A known compiler macro should be set");
...
bool TestResult()
{
    ...
    passed &= Expect(failure.GetError().GetCode() == ve::ErrorCode::NotFound, "Result<int> should preserve error code");
...
bool TestAssertions()
{
    ...
    passed &= Expect(std::string_view(captured.expression) == "value > 0", "Assertion expression should be captured");
    passed &= Expect(std::string_view(captured.message) == "value must be positive", "Assertion message should match");
...
bool TestUtilities()
{
    ...
    int cleanupCount = 0;
    {
        auto cleanup = ve::MakeScopeExit([&cleanupCount]()
        {
            ++cleanupCount;
        });
    }
