...
bool TestScheduleSingleJobWaitAndIsComplete()
{
    ...
    ve::Result<ve::JobHandle> result = jobs.Schedule(ve::JobDesc{"SingleJob"}, [&]()
    {
...
bool TestMultipleJobsExecute()
{
    ...
    {
        ve::Result<ve::JobHandle> result = jobs.Schedule(ve::JobDesc{"CountJob"}, [&]()
        {
...
bool TestSingleDependencyOrders()
{
    ...
    ve::Result<ve::JobHandle> second = first
        ? jobs.ScheduleAfter(ve::JobDesc{"Second"}, first.GetValue(), [&]()
          {
              std::lock_guard<std::mutex> lock(mutex);
              order.push_back(2);
          })
...
bool TestMultipleDependenciesOrder()
{
    ...
    ve::Result<ve::JobHandle> first = jobs.Schedule(ve::JobDesc{"FirstDependency"}, [&]()
    {
        releaseDependencies.Wait();
        completedDependencies.fetch_add(1, std::memory_order_acq_rel);
    });

    ve::Result<ve::JobHandle> second = jobs.Schedule(ve::JobDesc{"SecondDependency"}, [&]()
    {
        releaseDependencies.Wait();
        completedDependencies.fetch_add(1, std::memory_order_acq_rel);
    });
    ...
    ve::Result<ve::JobHandle> final = jobs.ScheduleAfter(
        ve::JobDesc{"AfterBoth"},
        dependencies,
        [&]()
        {
            observedDependencyCount.store(completedDependencies.load(std::memory_order_acquire), std::memory_order_release);
        });
    ...
    passed &= Expect(
        observedDependencyCount.load(std::memory_order_acquire) == 2,
...
bool TestWaitAll()
{
    ...
    {
        ve::Result<ve::JobHandle> result = jobs.Schedule(ve::JobDesc{"WaitAllJob"}, [&]()
        {
            counter.fetch_add(1, std::memory_order_acq_rel);
        });
...
bool TestHighPriorityRunsBeforeQueuedNormalJobs()
{
    ...
    {
        std::lock_guard<std::mutex> lock(mutex);
        passed &= Expect(order == std::vector<int>({2, 1}), "High-priority queued job should run before normal job");
    }
...
bool TestParallelForCoversEachItemOnce()
{
    ...
    ve::Result<ve::JobHandle> result = jobs.ParallelFor(
        ve::JobDesc{"VisitItems"},
        ItemCount,
        17,
        [&](ve::SizeT index)
        {
    ...
    ve::Result<ve::JobHandle> invalidBatch = jobs.ParallelFor(
        ve::JobDesc{"InvalidBatch"},
        ItemCount,
        0,
        [](ve::SizeT)
        {
    ...
    {
        passed &= Expect(
            invalidBatch.GetError().GetCode() == ve::ErrorCode::InvalidArgument,
...
bool TestShutdownWaitsForUnfinishedJob()
{
    ...
    ve::Thread releaser;
    passed &= ExpectOk(
        releaser.Start(ve::ThreadDesc{"ShutdownWaitReleaser"}, [&]()
        {
    ...
    jobs.Shutdown();
    passed &= Expect(jobCompleted.load(std::memory_order_acquire), "Shutdown should wait until unfinished job completes");
...
bool TestScheduleAfterShutdownFails()
{
    ...
    ve::JobSystem jobs;
    passed &= ExpectOk(jobs.Initialize(OneWorkerDesc()), "JobSystem should initialize before shutdown failure test");
    jobs.Shutdown();
