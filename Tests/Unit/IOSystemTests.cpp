...
bool TestInitializeAfterShutdownSucceeds()
{
    ...
    ioSystem.Shutdown();
    passed &= ExpectOk(ioSystem.Initialize(MakeIOSystemDesc()), "Standalone IOSystem should initialize after Shutdown");
    ioSystem.Shutdown();
...
bool TestScheduleBeforeInitializeAndAfterShutdownFails()
{
    ...
    passed &= ExpectOk(ioSystem.Initialize(MakeIOSystemDesc()), "IOSystem should initialize before shutdown failure test");
    ioSystem.Shutdown();
...
bool TestReadBinaryFileCompletes()
{
    ...
    passed &= ExpectOk(
        ve::FileSystem::WriteBinaryFile(path, bytes.data(), bytes.size()),
    ...
    passed &= Expect(result.data == std::vector<std::byte>(bytes.begin(), bytes.end()), "Completed read should match bytes");
...
bool TestWaitAll()
{
    ...
    passed &= ExpectOk(ve::FileSystem::WriteTextFile(secondPath, "second"), "Second WaitAll file should be written");
...
bool TestShutdownDrainsAcceptedReads()
{
    ...
    ve::IOSystem ioSystem;
    passed &= ExpectOk(ioSystem.Initialize(MakeIOSystemDesc()), "IOSystem should initialize for shutdown drain test");
    ...
    if (request)
    {
        passed &= Expect(ioSystem.IsComplete(request.GetValue()), "Shutdown should drain accepted reads before returning");
    }
...
bool TestInvalidPathRejected()
{
    ...
    {
        passed &= Expect(
            request.GetError().GetCode() == ve::ErrorCode::InvalidArgument,
