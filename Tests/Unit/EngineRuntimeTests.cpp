...
namespace
{
    ...
    bool TestRepeatedLifecycleFails()
    {
        ...
        passed &=
            Expect(secondLifecycle != ve::ErrorCode::None, "EngineRuntime should reject Initialize after Shutdown");
        passed &= Expect(secondLifecycle == ve::ErrorCode::InvalidState,
    ...
    bool TestCanScheduleJobThroughRuntime()
    {
        ...
        ve::EngineRuntime runtime;
        passed &=
            ExpectOk(runtime.Initialize(MakeRuntimeDesc()), "EngineRuntime should initialize for job access test");

        ve::AtomicInt32 value{0};
        ve::Result<ve::JobHandle> handleResult = runtime.GetJobSystem().Schedule(
            ve::JobDesc{"RuntimeJob"}, [&]() { value.store(17, std::memory_order_release); });
    ...
    bool TestCanReadThroughRuntimeIOSystem()
    {
        ...
        const ve::Path path =
            ve::FileSystem::GetCurrentWorkingDirectory() / "Generated/EngineRuntimeTests/IOSystem.bin";
        ...
        passed &= Expect(runtime.GetIOSystem().TryPopCompletedRead(result),
                         "Runtime-owned IOSystem should return completion");
    ...
    bool TestCanSubmitThroughRuntimeRenderSystem()
    {
        ...
        ve::EngineRuntime runtime;
        passed &=
            ExpectOk(runtime.Initialize(MakeRuntimeDesc()), "EngineRuntime should initialize for render access test");
        ...
        passed &= ExpectOk(runtime.GetRenderSystem().Submit(std::move(command)),
                           "Runtime-owned RenderSystem should accept commands");
        passed &= ExpectOk(runtime.GetRenderSystem().Flush(), "Runtime-owned RenderSystem should flush commands");
        passed &=
            Expect(value.load(std::memory_order_acquire) == 29, "Runtime-owned RenderSystem command should execute");
        passed &=
            Expect(executedThreadId.IsValid(), "Runtime-owned RenderSystem command should see a Render Thread id");
