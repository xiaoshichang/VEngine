...
namespace
{
    ...
    bool TestInitializeAndShutdown()
    {
        ...
        passed &= Expect(gameThreadSystem.GetCurrentPhase() == ve::GameThreadPhase::Uninitialized,
                         "New GameThreadSystem should start with Uninitialized phase");
        passed &=
            ExpectOk(gameThreadSystem.Initialize(MakeGameThreadSystemDesc()), "GameThreadSystem should initialize");
        passed &= Expect(gameThreadSystem.IsInitialized(), "Initialized GameThreadSystem should report initialized");
        passed &= Expect(gameThreadSystem.GetGameThreadId().IsValid(),
                         "Initialized GameThreadSystem should expose a Game Thread id");
        passed &= Expect(gameThreadSystem.GetGameThreadId() != ve::GetCurrentThreadId(),
    ...
    bool TestRepeatedInitializeWhileRunningFails()
    {
        ...
        ve::GameThreadSystem gameThreadSystem;
        passed &= ExpectOk(gameThreadSystem.Initialize(MakeGameThreadSystemDesc()),
    ...
    bool TestGameThreadOwnsFrameLoop()
    {
        ...
        passed &= Expect(WaitForCompletedFrameCount(gameThreadSystem, 2),
                         "GameThreadSystem should advance frames from its own loop");
        passed &= Expect(gameThreadSystem.GetFrameIndex() >= gameThreadSystem.GetCompletedFrameCount(),
    ...
    bool TestGameThreadOwnsTimeTick()
    {
        bool passed = true;
        ...
        ve::GameThreadSystem gameThreadSystem;
        passed &= ExpectOk(gameThreadSystem.Initialize(MakeGameThreadSystemDesc()),
                           "GameThreadSystem should initialize for Time tick ownership test");
        passed &= Expect(WaitForCompletedFrameCount(gameThreadSystem, 2),
                         "GameThreadSystem should complete frames while owning Time tick");

        gameThreadSystem.Shutdown();

        passed &=
            Expect(ve::Time::GetFrameIndex() >= 2, "Time frame index should advance from the Game Thread frame loop");

        return passed;
    }

    bool TestRenderSystemConnectionValidation()
    {
        bool passed = true;

        ve::GameThreadSystem gameThreadSystem;
        passed &= ExpectOk(gameThreadSystem.Initialize(MakeGameThreadSystemDesc()),
                           "GameThreadSystem should initialize for RenderSystem connection validation test");

        const ve::ErrorCode nullResult = gameThreadSystem.SetRenderSystem(nullptr);
        passed &= Expect(nullResult == ve::ErrorCode::InvalidArgument, "Null RenderSystem connection should fail");
    ...
    bool TestGameThreadAccessRule()
    {
        ...
        passed &=
            Expect(!gameThreadSystem.CheckGameThreadAccess(), "Calling thread should not be the dedicated Game Thread");
        passed &= Expect(gameThreadSystem.GetGameThreadId() != ve::GetCurrentThreadId(),
    ...
    bool TestEngineRuntimeOwnsGameThreadSystem()
    {
        ...
        passed &= ExpectOk(runtime.Initialize(MakeRuntimeDesc()),
                           "EngineRuntime should initialize for GameThreadSystem test");
        passed &= Expect(runtime.GetGameThreadSystem().IsInitialized(),
                         "Runtime-owned GameThreadSystem should be initialized");
        passed &= Expect(runtime.GetGameThreadSystem().GetGameThreadId().IsValid(),
                         "Runtime-owned GameThreadSystem should expose a Game Thread id");
        passed &= Expect(WaitForCompletedFrameCount(runtime.GetGameThreadSystem(), 2),
...
int main()
{
    ...
    passed &= TestGameThreadOwnsTimeTick();
    passed &= TestRenderSystemConnectionValidation();
