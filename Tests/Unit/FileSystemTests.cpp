...
namespace
{
    constexpr std::string_view ChineseRelativePath = "\xE4\xB8\xAD\xE6\x96\x87/\xE6\xB5\x8B\xE8\xAF\x95.txt";
    ...
    bool ContainsEntry(const std::vector<ve::FileSystem::DirectoryEntry>& entries,
                       std::string_view name,
                       ve::FileSystem::DirectoryEntryType type)
    {
        return std::any_of(entries.begin(),
                           entries.end(),
                           [name, type](const ve::FileSystem::DirectoryEntry& entry)
    ...
    bool TestPathNormalization()
    {
        ...
        passed &= Expect(ve::Path("Assets\\Textures\\./Brick.png").GetString() == "Assets/Textures/Brick.png",
                         "Path should normalize slashes and dot segments");
        passed &= Expect(ve::Path("C:\\VEngine\\..\\Game\\file.txt").GetString() == "C:/Game/file.txt",
                         "Path should preserve Windows drive roots and resolve parent segments");
        passed &= Expect(ve::Path("../Assets/./Mesh.fbx").GetString() == "../Assets/Mesh.fbx",
        ...
        passed &= Expect(ve::Path("//server/share/Folder/../File.txt").GetString() == "//server/share/File.txt",
                         "UNC path should preserve server and share root");
        passed &= Expect(ve::Path("//server//share//Folder").GetString() == "//server/share/Folder",
                         "UNC path should collapse repeated separators after the server name");
        passed &= Expect(ve::Path("//server/share").GetParentPath().GetString() == "//server/share",
                         "UNC share root should parent to itself");
        passed &= Expect(ve::Path("/").GetParentPath().GetString() == "/", "Unix root should parent to itself");
        passed &=
            Expect(ve::Path("C:/").GetParentPath().GetString() == "C:/", "Windows drive root should parent to itself");
    ...
    bool TestPathQueriesAndJoining()
    {
        ...
        passed &= Expect(joined.GetString() == "C:/Project/Assets/Textures/Brick.png",
                         "Join should append relative paths and normalize separators");
        passed &= Expect((base / ve::Path("D:/Other/File.txt")).GetString() == "D:/Other/File.txt",
        ...
        passed &=
            Expect(ve::Path(".gitignore").GetExtension().empty(), "Dotfile without suffix should not have extension");
        passed &=
            Expect(ve::Path("Assets").GetParentPath().IsEmpty(), "Single relative segment should have empty parent");
    ...
    bool TestTextAndBinaryFiles()
    {
        ...
        passed &= ExpectOk(ve::FileSystem::WriteTextFile(textPath, "line1\nline2"),
                           "WriteTextFile should create parent dirs");
        ...
        if (textResult)
        {
            passed &= Expect(textResult.GetValue() == "line1\nline2",
                             "ReadTextFile should preserve UTF-8 bytes and newlines");
        }
        ...
        passed &= ExpectOk(ve::FileSystem::WriteBinaryFile(binaryPath, sourceBytes.data(), sourceBytes.size()),
    ...
    bool TestUtf8TextAndChinesePath()
    {
        ...
        passed &= ExpectOk(ve::FileSystem::WriteBinaryFile(bomPath, bomText.data(), bomText.size()),
        ...
        passed &= ExpectOk(ve::FileSystem::WriteTextFile(chinesePath, "Chinese path smoke test"),
        ...
        if (chineseResult)
        {
            passed &= Expect(chineseResult.GetValue() == "Chinese path smoke test",
    ...
    bool TestDirectoryOperations()
    {
        ...
        passed &=
            ExpectOk(ve::FileSystem::CreateDirectories(childDirectory), "CreateDirectories should create nested dirs");
        passed &= ExpectOk(ve::FileSystem::WriteTextFile(filePath, "entry"), "WriteTextFile should create a list file");
        passed &=
            ExpectOk(ve::FileSystem::WriteTextFile(nestedFile, "nested"), "WriteTextFile should create a nested file");
        ...
        const ve::Result<std::vector<ve::FileSystem::DirectoryEntry>> entriesResult =
            ve::FileSystem::ListDirectory(listRoot);
        ...
        if (entriesResult)
        {
            ...
            passed &= Expect(ContainsEntry(entries, "Child", ve::FileSystem::DirectoryEntryType::Directory),
                             "ListDirectory should include immediate child directory");
            passed &= Expect(ContainsEntry(entries, "Entry.txt", ve::FileSystem::DirectoryEntryType::File),
                             "ListDirectory should include immediate child file");
            passed &= Expect(!ContainsEntry(entries, "Nested.txt", ve::FileSystem::DirectoryEntryType::File),
    ...
    bool TestProjectRootResolution()
    {
        ...
        passed &=
            Expect(ve::FileSystem::GetProjectRoot() == root, "SetProjectRoot should store normalized project root");
        passed &= Expect(ve::FileSystem::ResolveProjectPath(ve::Path("Assets/Mesh.fbx")) == root / "Assets/Mesh.fbx",
                         "ResolveProjectPath should resolve relative paths against project root");
        passed &= Expect(ve::FileSystem::ResolveProjectPath(ve::Path("C:/Absolute/File.txt")).GetString() ==
                             "C:/Absolute/File.txt",
    ...
    bool TestMissingFileError()
    {
        ...
        if (!missingResult)
        {
            passed &= Expect(missingResult.GetError().GetCode() == ve::ErrorCode::NotFound,
