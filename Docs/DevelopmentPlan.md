...
- GPU-driven VSM is the only implementation. D3D11 and D3D12 run the mandatory GPU path. Metal currently has no VSM and terminates during VSM graph construction until that backend is implemented.

One frame-local renderer consumes the whole `RenderViewFamily` and owns its single FrameGraph. Generic per-view depth pre-passes feed focused VSM request, residency, raster, finalization, and statistics-copy passes. The `RenderSystem`-owned `VirtualShadowManager` keeps one persistent cache per `RTScene`; all views share that scene's physical atlas, metadata, allocator, and statistics while retaining stable view IDs and distinct page-table slices. FrameGraph resource versions define the execution order. The legacy pre-render scheduler and monolithic GPU pipeline have been removed.

GPU cache reset requests are consumed immediately and independently from resource-recreation resets. Dynamic caster revisions no longer trigger a whole-pool reset: the CPU produces a compact list of absolute XY page identities and clipmap levels from changed caster bounds, and the GPU marks only matching persistent physical pages dirty. These identities are not clipped to the current camera working region and ignore the depth epoch during invalidation matching, so dormant resident pages cannot retain stale shadows after the camera moves away and later returns. Full mapping resets remain reserved for incompatible changes such as a directional-light basis change, shadow-distance change, device/resource recreation, or explicit cache reset. Overflow conservatively dirties all resident content without discarding mappings.

VSM has no disabled, placeholder, fallback-resource, prepared-frame transaction, rollback, retry, or same-frame recovery model. Every rendered family requires a valid scene, supported backend, shadow-casting directional light, camera and receiver depth for each view, and all real VSM resources. Renderer and pass orchestration APIs are `void`; violations and graph, resource, recording, submission, or presentation failures log an error and terminate through the always-on assertion path. Statistics use a one-way lifecycle: graph copy, successful-submit fence association, then consumption after that exact fence completes and before the slot is reused.
