...
namespace ve
{
                                               ...
                                               RenderPassContext& context)
    {
        ...
        for (SizeT itemIndex = 0; itemIndex < items.size(); ++itemIndex)
        {
            ...
            if (item == nullptr)
            {
                VE_LOG_WARN("%s skipping null render item at queue index %zu.", TransparentScenePassName, itemIndex);
                continue;
            }
            ...
            {
                VE_LOG_WARN("%s skipping render item at queue index %zu due to missing mesh data.", TransparentScenePassName, itemIndex);
                continue;
            }
    ...
    ErrorCode TransparentSceneRenderPass::EnsurePipeline(RenderPassContext& context)
    {
        std::shared_ptr<RTShaderResource> shaderResource = FindFirstShaderResource(context.rendererData.transparentItems);
        VE_ASSERT_ALWAYS_MESSAGE(shaderResource != nullptr, "Transparent scene pass requires an initialized material shader resource.");
        ...
        {
            VE_ASSERT_ALWAYS_MESSAGE(false, "Transparent scene pass material shader resource is not initialized.");
            return ErrorCode::InvalidState;
        }
        ...
        {
            VE_ASSERT_ALWAYS_MESSAGE(false, "Transparent scene pass failed to create its graphics pipeline.");
    ...
    bool TransparentSceneRenderPass::BindMaterialUniform(RenderPassContext& context, const RTRenderItem& item)
    {
        const auto materialResource = std::dynamic_pointer_cast<RTMaterialResource>(item.GetMaterialResource());
        ...
        VE_ASSERT_ALWAYS_MESSAGE(false, "Transparent scene pass requires a material resource with an initialized uniform buffer.");
        return false;
    }
