...
///
/// ThreadId is intentionally not a platform handle and is not suitable for native API calls. It exists so engine code
/// can compare and report thread identities without depending on std::thread::id, HANDLE, or pthread_t in public APIs.
struct ThreadId
{
    /// Process-local diagnostic id value. A value of zero means "no thread"; ids may be reused after a thread exits.
    UInt64 value = 0;
...
///
/// The first-stage descriptor only stores a diagnostic name. Future priority and affinity fields should be added here
/// without exposing platform handles or changing normal Thread call sites.
struct ThreadDesc
{
...
///
/// Thread is non-copyable and non-movable so ownership and shutdown order remain explicit. A Thread object owns at most
/// one joinable thread at a time. Owners must call Join() or Detach() before destruction.
...
public:
    ...
    ///
    /// Returns true when a joinable thread was joined. Returns false when this Thread is not joinable or when joining
    /// would be invalid, such as joining from the same thread. Invalid joins are API misuse and assert in debug builds.
    [[nodiscard]] bool Join();

    /// Detaches the owned thread.
    ///
    /// Returns true when a joinable thread was detached. Returns false when this Thread is not joinable. Invalid detach
    /// calls are API misuse and assert in debug builds.
...
///
/// This helper is a thin VEngine-owned wrapper over std::this_thread::sleep_for so call sites do not depend directly on
/// standard-library threading APIs.
