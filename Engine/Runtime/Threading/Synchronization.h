...
namespace ve
{
    ...
    ///
    /// Mutex is non-copyable and non-movable. It may block in Lock(). The same thread must not call Lock() twice
    /// without an intervening Unlock(); use RecursiveMutex only when recursive acquisition is truly required.
    ...
    public:
        ...
        ///
        /// Returns true when the calling thread acquired the mutex. Returns false when another thread currently owns
        /// it.
    ...
    ///
    /// RecursiveMutex is non-copyable and non-movable. The owning thread may call Lock() multiple times, but it must
    /// call Unlock() once for each successful Lock() or TryLock(). Prefer Mutex unless recursive acquisition is
    /// required.
    ...
    ///
    /// UniqueLock owns at most one Mutex lock. It may be constructed locked or deferred. ConditionVariable waits
    /// require a UniqueLock so the wait operation can atomically release and reacquire the mutex.
    ...
    ///
    /// ManualResetEvent is non-copyable and non-movable. Set() wakes all current waiters and lets future waiters pass
    /// until Reset() clears the signaled state.
