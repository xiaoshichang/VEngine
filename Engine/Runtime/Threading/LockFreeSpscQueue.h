...
///
/// This queue is for exactly one producer thread and exactly one consumer thread. Using multiple producers or multiple
/// consumers is invalid. Capacity is fixed at compile time, and the queue performs no runtime allocation.
///
...
public:
    ...
    {
        // Acquire both indices so this diagnostic query observes the latest published producer and consumer progress.
