...
namespace ve
{
    ...
    ///
    /// Priority is an ordering hint for queued requests. A high-priority request is selected before queued normal
    /// requests, but it does not interrupt a file read that is already running on the IO Thread.
    ...
    ///
    /// A handle may be copied freely and can outlive the IOSystem object that produced it. Waiting on an invalid handle
    /// is a no-op, and invalid handles are considered complete.
    ...
    ///
    /// IOSystem owns one dedicated IO Thread. It accepts binary file read requests, executes them through FileSystem on
    /// the IO Thread, and exposes completed results through polling. It is not a general worker pool and does not
    /// invoke caller callbacks from the IO Thread.
    ...
    public:
        ...
        ///
        /// Shutdown is a no-op when the system is not initialized. Unconsumed completed results are discarded during
        /// final teardown because the service is leaving its active runtime lifetime.
        ...
        ///
        /// A successful return means the request was accepted. File-not-found and read errors are reported later
        /// through the completed read result rather than through this scheduling result.
        ...
        ///
        /// Returns true and writes outResult when a result was available. Returns false when the completion queue is
        /// empty. Each completed read result is popped at most once. The completion queue is global to this IOSystem;
        /// results are not dispatched back to the request submitter. The thread that calls this function owns handling
