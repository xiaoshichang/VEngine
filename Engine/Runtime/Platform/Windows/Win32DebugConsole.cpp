...
namespace
{
    ...
    {
        // The input thread can block in ReadConsoleInput until process exit, so the shared state intentionally outlives
        // it.
    ...
    void WriteAt(HANDLE outputHandle, SHORT x, SHORT y, std::wstring_view text)
    {
        ...
        DWORD written = 0;
        WriteConsoleOutputCharacterW(outputHandle, text.data(), static_cast<DWORD>(text.size()), COORD{x, y}, &written);
    }
