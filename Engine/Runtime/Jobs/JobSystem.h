...
///
/// Priority is an ordering hint inside the worker-thread queues. A high-priority job is selected before queued normal
/// jobs, but it does not interrupt work that is already running on a worker.
...
///
/// The name is a diagnostic label for debugging and future profiling. Empty names are allowed. The first implementation
/// stores the name but does not require platform-level task naming support.
struct JobDesc
{
    std::string name;
...
///
/// A handle may be copied freely and can outlive the JobSystem object that produced it. It does not own a worker thread
/// and does not expose the callable. Invalid handles are treated as already complete when used as dependencies.
class JobHandle
{
public:
    JobHandle() = default;
...
/// continuations, waiting, two priority queues, and ParallelFor batching. Jobs run on worker threads only; they must not
/// call RHI directly or access live GameObject state without a higher-level synchronization plan.
...
public:
    ...
    ///
    /// The callable is copied or moved into the job state and invoked once on a worker thread. It must be callable as
    /// `void()`. Captured references must remain valid until the job completes.
    ...
    ///
    /// Invalid or already-complete dependencies do not delay the job. The dependency only controls ordering; it does not
    /// transfer ownership of captured data or make unsafe shared state access safe.
    template <typename Callable>
    [[nodiscard]] Result<JobHandle> ScheduleAfter(
        const JobDesc& desc,
    ...
    ///
    /// The span is consumed during the call and does not need to remain valid afterward. Invalid handles are ignored.
    template <typename Callable>
    [[nodiscard]] Result<JobHandle> ScheduleAfter(
        const JobDesc& desc,
    ...
    ///
    /// The callable is invoked as `void(SizeT index)`. batchSize controls how many indices are grouped into one child
    /// job. The returned handle completes after every batch job completes. Each index must be independent unless the
    /// caller provides its own synchronization for shared state.
    template <typename Callable>
    [[nodiscard]] Result<JobHandle> ParallelFor(
        const JobDesc& desc,
        SizeT itemCount,
        SizeT batchSize,
        Callable&& callable)
    ...
    ///
    /// Waiting on an invalid handle returns immediately. Waiting from inside worker jobs is discouraged because it can
    /// reduce available parallelism and can deadlock if callers create cyclic waiting outside the dependency API.
...
private:
    ...
    [[nodiscard]] Result<JobHandle> ScheduleFunction(
        const JobDesc& desc,
        Result<JobFunction> functionResult,
        std::span<const JobHandle> dependencies);

    [[nodiscard]] Result<JobHandle> ParallelForFunction(
        const JobDesc& desc,
        SizeT itemCount,
        SizeT batchSize,
        IndexedJobFunction function);
