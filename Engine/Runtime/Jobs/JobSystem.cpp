...
void MarkJobComplete(JobSystemImpl& impl, const std::shared_ptr<JobState>& state)
{
...

    {
        ...
        {
            VE_ASSERT_MESSAGE(
                continuation->pendingDependencies > 0,
...
void WorkerLoop(JobSystemImpl& impl)
{
    for (;;)
    {
...

        {
            ...
            impl.schedulerCondition.wait(lock, [&]()
            {
                return impl.stopping || HasRunnableJob(impl);
            });
...
{
    {
        ...
        impl_->allJobsCompleteCondition.wait(lock, [&]()
        {
            return impl_->incompleteJobCount == 0;
        });
    }
...
Result<JobHandle> JobSystem::ScheduleFunction(
    const JobDesc& desc,
...
Result<JobHandle> JobSystem::ParallelForFunction(
    const JobDesc& desc,
    SizeT itemCount,
    SizeT batchSize,
    IndexedJobFunction function)
{
    ...
    for (SizeT batchIndex = 0; batchIndex < batchCount; ++batchIndex)
    {
        ...
        Result<JobHandle> batchResult = Schedule(batchDesc, [sharedFunction, begin, end]()
        {
...
void JobSystem::Wait(const JobHandle& handle) const
{
    ...
    handle.state_->completionCondition.wait(lock, [&]()
    {
