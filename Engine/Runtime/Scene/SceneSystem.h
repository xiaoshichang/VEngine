...
///
/// SceneSystem is the runtime service boundary for live GameObject state. It owns one active Scene object and updates it
/// on the Scene Thread. Future render extraction should copy or command scene data across thread boundaries instead of
/// letting Render Thread code read GameObject state directly.
...
public:
    ...
    ///
    /// Shutdown is a no-op when the system is not initialized. The active Scene is preserved until the SceneSystem is
    /// destroyed or ReplaceScene() is called, so callers can inspect or explicitly clear it after stopping updates.
