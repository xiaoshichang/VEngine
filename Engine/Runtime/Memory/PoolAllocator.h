...
///
/// blockSize is the minimum payload size returned by each allocation. blockCount is the number of blocks in the pool.
/// alignment must be a non-zero power of two and is applied to every returned block.
...
///
/// allocatedBlockCount reports the current live allocation count. highWaterMark is the maximum live allocation count
/// observed since the most recent successful Initialize(). totalBytes is the backing storage size owned by the pool.
...
///
/// PoolAllocator owns one contiguous backing allocation and divides it into equally sized, aligned blocks. Allocate()
/// returns one free block or nullptr when the pool is exhausted. Free() returns a block to the pool and rejects null
/// pointers, foreign pointers, misaligned pointers, and double frees.
///
/// Usage:
/// ```cpp
...
/// ```
///
/// PoolAllocator is not thread-safe. Callers must provide external synchronization when the same allocator can be used
...
public:
    ...
    ///
    /// The caller is responsible for destroying any live objects before Shutdown(). This function is safe to call on an
    ...
    ///
    /// Returns false for null, foreign, misaligned, uninitialized, or already-free pointers. Free() does not call object
    /// destructors; callers must destroy constructed objects before returning their storage.
