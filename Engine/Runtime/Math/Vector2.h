...
///
/// `Vector2` is a small value type: pass it by value, store it directly in components, and avoid heap allocation for
/// individual vectors. If many objects containing vectors need stable allocation, allocate those owning objects through
/// an allocator such as `PoolAllocator`.
class Vector2
{
public:
