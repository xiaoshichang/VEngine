...
public:
    ...
    ///
    /// The axis is normalized internally. A near-zero axis returns identity so callers can safely pass optional axes.
    [[nodiscard]] static Quaternion FromAxisAngle(const Vector3& axis, Float32 radians) noexcept
    {
        ...
        return Quaternion(
                   normalizedAxis.GetX() * s,
                   normalizedAxis.GetY() * s,
                   normalizedAxis.GetZ() * s,
                   Cos(halfAngle))
            .Normalized();
    }
    ...
    [[nodiscard]] Quaternion Normalized(Float32 epsilon = Math::DefaultEpsilon) const noexcept
    {
        ...
        return length > epsilon ? Quaternion(x_ / length, y_ / length, z_ / length, w_ / length)
                                : Quaternion::Identity();
    }
